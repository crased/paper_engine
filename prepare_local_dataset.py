#!/usr/bin/env python3
"""
Convert local JSON annotations to YOLO format

Note: This module handles legacy JSON annotation files from the dataset/
directory.  New annotations are created directly in YOLO format via the
built-in annotation tool (review_results.py).
"""
...
def convert_json_to_yolo(
    json_file, output_labels_dir, class_mapping, label_mapping=None
):
    """
    Convert a single legacy JSON annotation file to YOLO format.
    ...
    # Parse the local file path format
    ...
    for annotation in annotations:
        ...
        # JSON annotations use percentage coordinates (0-100)
...
def prepare_dataset(
    dataset_dir=None, screenshots_dir=None, output_dir=None, train_split=None
):
    """
    Prepare YOLO dataset from legacy JSON annotations.

    Args:
        dataset_dir: Directory containing JSON annotation files (default: from config)
    ...
    for json_file in json_files:
        image_filename = convert_json_to_yolo(
...
if __name__ == "__main__":
    ...
    print(
        f"  Custom grouping: {'Enabled' if config.ENABLE_CUSTOM_GROUPING else 'Disabled (auto-discovery)'}"
    )
