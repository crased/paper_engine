...
def get_api_key():
    ...
    Lookup order:
        1. OS keyring via KEYRING_SERVICE + KEYRING_USER from env/.env
        2. API_KEY environment variable (legacy fallback)

    Returns:
        str or None: The API key, or None if not found / placeholder.
    """
    ...
    # 1. Try keyring
    service = os.environ.get("KEYRING_SERVICE")
    user = os.environ.get("KEYRING_USER")
    if service and user:
        try:
            import keyring
            ...
            print("Warning: keyring package not installed. Run: pip install keyring")
        except Exception as e:
            print(f"Warning: keyring lookup failed: {e}")

    # 2. Legacy fallback: raw API_KEY in env
...
def store_api_key(api_key):
    ...
    Uses KEYRING_SERVICE and KEYRING_USER from .env to determine
    ...
    if not service or not user:
        print("Warning: KEYRING_SERVICE/KEYRING_USER not set in .env")
        return False
    ...
    try:
        ...
        print(f"Warning: failed to store key in keyring: {e}")
        return False


# -- Cloud-credential keyring namespace --
_CLOUD_KEYRING_SERVICE = "PaperEngine"

# Fields that contain secrets (tokens, keys, connection strings).
# Matched by substring in the INI key name.
_SENSITIVE_SUBSTRINGS = ("key", "secret", "token", "connection_string")


def is_sensitive_field(key: str) -> bool:
    """Return True if *key* names a secret that should live in the OS keyring."""
    k = key.lower()
    return any(sub in k for sub in _SENSITIVE_SUBSTRINGS)


def store_secret(key: str, value: str) -> bool:
    """Store a cloud credential in the OS keyring.

    Secrets are stored under service='PaperEngine' with username='cloud/<key>'.

    Args:
        key:   The INI-style field name (e.g. 's3_secret_key').
    ...
    try:
        ...
        keyring.set_password(_CLOUD_KEYRING_SERVICE, f"cloud/{key}", value)
        return True
    except ImportError:
        print("Warning: keyring package not installed. Run: pip install keyring")
        return False
    except Exception as e:
        print(f"Warning: failed to store secret '{key}' in keyring: {e}")
        return False


def get_secret(key: str) -> str:
    """Retrieve a cloud credential from the OS keyring.

    Args:
        key: The INI-style field name (e.g. 's3_secret_key').

    Returns:
        The secret string, or '' if not found / keyring unavailable.
    """
    try:
        import keyring

        val = keyring.get_password(_CLOUD_KEYRING_SERVICE, f"cloud/{key}")
        return (val or "").strip()
    except ImportError:
        return ""
    except Exception:
...
def delete_secret(key: str) -> bool:
    """Remove a cloud credential from the OS keyring.

    Args:
        key: The INI-style field name.

    Returns:
        True on success or if the entry didn't exist.
    """
    try:
        import keyring

        keyring.delete_password(_CLOUD_KEYRING_SERVICE, f"cloud/{key}")
        return True
    except ImportError:
        return False
    except Exception:
...
def delete_last_screenshot(screenshots_dir="screenshots/captures", count=3):
    """Delete the most recent screenshots from the screenshots directory.

    Args:
        screenshots_dir: Path to screenshots directory (default: "screenshots/captures")
        count: How many of the most recent screenshots to delete (default: 3).

    Returns:
        int: Number of screenshots actually deleted.
    """
    ...
    # Never delete the last remaining screenshot — keep at least one.
    deletable = len(screenshot_files) - 1
    if deletable <= 0:
        print("Only one screenshot exists, skipping deletion.")
        return 0

    to_delete = min(count, deletable)

    # Sort by filename (contains timestamp: screenshot_YYYYMMDD_HHMMSS.png)
    # This is more reliable than modification time. Take the newest `to_delete`.
    screenshot_files.sort()
    targets = screenshot_files[-to_delete:]

    deleted = 0
    for target in reversed(targets):  # newest first
        try:
            target.unlink()
            print(f"✓ Deleted screenshot: {target.name}")
            deleted += 1
        except FileNotFoundError:
            print(f"Screenshot file disappeared: {target.name}")
        except PermissionError:
            print(f"Permission denied deleting: {target.name}")
        except Exception as e:
            print(f"Unexpected error deleting screenshot: {e}")

    print(f"Deleted {deleted} of the {to_delete} most recent screenshot(s).")
    return deleted
