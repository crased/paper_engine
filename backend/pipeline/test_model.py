...
def _resolve_model_path():
    """
    Find the most recently trained model by scanning bot_logic/models/ —
    where training writes its runs (via project= in training_conf).

    Order of preference: newest dir matching the configured
    model_output_name prefix, then the newest model of any name, then the
    configured best_model_path fallback.
    ...
    try:
        ...
        configured = "bot_logic/models/paper_engine_model/weights/best.pt"
        model_name = "paper_engine_model"

    if models_dir.is_dir():
        # Prefer the configured model family, then fall back to any model.
        for pattern in (f"{model_name}*/weights/best.pt", "*/weights/best.pt"):
            candidates = sorted(
            ...
            if candidates:
                chosen = candidates[0]
                configured_resolved = (
                    (project_root / configured).resolve() if configured else None
                )
                if configured_resolved and chosen.resolve() != configured_resolved:
    ...
    return (
        str(configured)
        if configured
        else "bot_logic/models/paper_engine_model/weights/best.pt"
    )
