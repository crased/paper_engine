...
def dataset_summary(dataset_dir=None):
    ...
    dataset_dir = Path(dataset_dir) if dataset_dir else DATA_ROOT / "yolo_dataset"
...
def build_cuphead_remap(source_class_names):
    ...
    for src_id, src_name in source_class_names.items():
        ...
        tokens = set(src_lower.split("_"))
        matched = False
        for target_id, keywords in _REMAP_RULES:
            for kw in keywords:
                # Single-word keywords must match a whole token, so "ring"
                # no longer matches "herring" and "king" no longer matches
                # "parking". Multi-word keywords (e.g. "cloud_platform")
                # still match as a substring of the normalized name.
                hit = (kw in src_lower) if "_" in kw else (kw in tokens)
                if hit:
