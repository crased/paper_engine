...
_DATA_ROOT = _PROJECT_ROOT.parent  # repo root: data dirs live here; code in backend/
...
class GameplayRecorder:
    ...
    def __init__(
        ...
        if output_root is None:
            self._output_root = _DATA_ROOT / "recordings" / "sessions"
        else:
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Label for the session dir / annotator context (e.g. EnterTheGungeon). "
        "Required in --no-memory mode for non-Cuphead games.",
    )
    parser.add_argument(
    ...
    if not args.no_memory:
        ...
        game_name = args.game_name or "unknown"
        if args.launch:
            # Launch game but don't attach memory (Cuphead-only launcher)
            from tools.cuphead_memory import CupheadMemory
    ...
    try:
        while True:
            ...
            if captured:
                # Print a live status line on every captured frame
