...
_DATA_ROOT = _PROJECT_ROOT.parent  # repo root: data dirs live here
...
def find_latest_session() -> Optional[Path]:
    ...
    sessions_dir = _DATA_ROOT / "recordings" / "sessions"
...
def scan_wine_prefix(prefix_dir: Path, log_fn=None) -> Dict[str, Any]:
    ...
    if tracked.exists():
        try:
            files = [ln.strip() for ln in tracked.read_text().splitlines() if ln.strip()]
...
def scan_dxvk_logs(
    ...
    # Search locations for DXVK logs. Game-local dirs are trusted; shared
    # dirs like /tmp hold logs from *every* game, so we additionally require
    # the filename to start with this game's name token to avoid leaking
    # unrelated games' logs into the LLM prompt.
    search_dirs: List[Tuple[Path, bool]] = [(game_dir, False)]
    for sub in ("Binaries/Win64", "Binaries/Win32", "bin", "bin/x64"):
        d = game_dir / sub
        if d.is_dir():
            search_dirs.append((d, False))
    # /tmp is a common DXVK log location, but shared across all games.
    tmp = Path("/tmp")
    if tmp.is_dir():
        search_dirs.append((tmp, True))

    # Name token used to disambiguate this game's logs in shared dirs.
    game_token = re.sub(r"[^a-z0-9]", "", game_dir.name.lower())

    log_patterns = ["dxvk*.log", "d3d11*.log", "dxgi*.log", "vkd3d*.log"]

    total_chars = 0
    for search_dir, shared in search_dirs:
        for pattern in log_patterns:
            for log_path in sorted(search_dir.glob(pattern)):
                if not log_path.is_file():
                    continue
                if shared and game_token:
                    fname = re.sub(r"[^a-z0-9]", "", log_path.stem.lower())
                    if not fname.startswith(game_token):
                        continue
                try:
