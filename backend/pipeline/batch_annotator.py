...
def _build_annotation_prompt(
    ...
    When ``game_name`` is known and the current scene has a stored context
    description, a per-scene prompt block is appended to enrich annotation.
    """
    ...
    scene_name = memory_state.get("scene_name", "")
    if game_name and scene_name:
        try:
            from pipeline.scene_context import get_scene_prompt_block

            scene_block = get_scene_prompt_block(game_name, scene_name)
            if scene_block:
                prompt = f"{prompt}\n\n{scene_block}"
        except Exception as exc:
            logger.debug("Scene context unavailable for %s: %s", scene_name, exc)
...
def annotate_session(
    ...
    if yolo_dataset_path is None:
        yolo_dataset_path = _DATA_ROOT / "yolo_dataset"
    ...
    game_name = session_info.get("game", "")
    ...
    if not dry_run and use_examples:
