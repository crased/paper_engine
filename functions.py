...
def _load_custom_filters():
   ...
   try:
       ...
       if custom_excluded or custom_position:
           print(f"✓ Loaded {len(custom_excluded)} custom substring filters and {len(custom_position)} position filters")
...
def path_finder(game_path):
   ...
   DEFAULT_POSITION_FILTERS = (
       ('unins', 0),    # Inno Setup uninstallers: unins000.exe, unins001.exe (industry standard)
       ('setup', 0),    # Common installer: setup.exe (default for most installer tools)
       ('install', 0),  # Installers: install.exe, installer.exe
       ('vcredist', 0), # Visual C++ redistributables: vcredist_x64.exe, vcredist_x86.exe
   ...
   excluded_patterns = list(DEFAULT_EXCLUDED_PATTERNS) + custom_excluded
   position_filters = list(DEFAULT_POSITION_FILTERS) + custom_position
